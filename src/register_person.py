...
def update_person_to_admin(transaction_executor,person_id):
    ...
    try:
        ...
        logger.exception("Problem arised while making person with person id :{} as admin.".format(person_id))
...
def create_mcg_request(transaction_executor,document_id,person_id, request_type):
    ...
    if mcg_request_already_sent(transaction_executor, document_id):
        raise Exception("Request already sent for document id : {}".format(document_id))
    else:
...
def register_new_user_with_scentity(transaction_executor, person, new_sc_entity):
    ...
    if get_scentityid_from_personid(transaction_executor, person_id):
        raise Exception("Person with personId '{}' already belongs to a SC Entity".format(person_id))
    else:
        ...
        if lookup_scentity(transaction_executor, new_sc_entity):
            ...
            if get_document_superadmin_approval_status(transaction_executor,Constants.SCENTITY_TABLE_NAME, scentity_id):
                if req_already_sent(transaction_executor, person_id) == True :
                    raise Exception("Please wait for your company admin to approve the request.")
                else:
                    joining_request_id = create_req_to_join_scentity(transaction_executor,new_sc_entity,employee_id,person_id)
                    send_request_to_company(transaction_executor,joining_request_id, new_sc_entity)
            ...
            if req_already_sent(transaction_executor, person_id) == True :
                ...
                try:
                    ...
                    logger.info(" ================= P E R S O N ======= A N D ===== S C E N T I T Y ========= R E G I S T R A T I O N ========= I N I T I A T E D=========")
                    return person_id, sc_entity_id, mcg_request_id
                except StopIteration:
                    raise Exception('Problem occurred while inserting Scentity, please review the results.')
