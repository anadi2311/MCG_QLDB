                ...
                for container_id in container_ids[0]:
                    if isContainerSafe(transaction_executor,container_id):
                        ...
                        if isPicked[0] == 0:
                            ...
                            create_lorry_reciept(transaction_executor, actual_sc_entity_id,truck_carrier_person_id,pick_up_location[0],delivery_location[0], consignee_id[0],consignee_name[0], True)

                            logger.info(" =========== L O C A L ====== T R A N S P O R T ======== I N I T I A T E D ============= ")                            
                        else:
                            raise Exception("Order Already Picked")
                    else:
                        logger.info(" Container is not Safe for Delivery!")
            else:
